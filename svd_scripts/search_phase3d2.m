
clear all


tmin = 0e-3;
tmax = 1e-3;
dt   = 2e-6;
t = tmin:dt:tmax;
Nt= length(t);

% coil positions
theta   = 0;
phi     = 6/180*pi;

% reproduce example of Raju et al
f1      = 1;
n1      = 0;
PHI10   = 20/180*pi;
% PHI10   = rand*2*pi;
wnorm   = 100.23984723904e+3 *2 * pi;
% wnorm   = 100.0e+3 *2 * pi;
w1      = 0.12 * wnorm

f2      = 1;
n2      = 0;
PHI20   = 45/180*pi;
% PHI20   = rand*2*pi;
w2      = 0.18 * wnorm;

f3      = 1;
n3      = 0;
PHI30   = 100/180*pi;
% PHI30   = rand*2*pi;
w3      = w1 + w2;

noise.sigma = 0.2;
noise.mu    = 0.0;

disp(['Normalization =================================']);
winl    = Nt;
winl    = Nt-1;
norm    = spec_norm(winl)

Nsamples= 40;

xmd.omt(1).data           = 1 ;
xmd.omt(1).phi            = 0.1047;
xmd.omt(1).signal(:,1)    = t.';
for kk = 1:Nsamples

    DELTA_t = rand * tmax;
    PHI1    = PHI10  + DELTA_t * w1;
    PHI2    = PHI20  + DELTA_t * w2;
    PHI3    = PHI30  + DELTA_t * w3;

    xmd.omt(1).signal(1:Nt,2) = 0.0;

    for i=1:Nt

        xmd.omt(1).signal(i,2) =  f1 * cos( - n1* xmd.omt(1).phi  + w1*t(i) + PHI1 ) + ...
                                  f2 * cos( - n2* xmd.omt(1).phi  + w2*t(i) + PHI2 ) + ...
                                  f3 * cos( - n3* xmd.omt(1).phi  + w3*t(i) + PHI3 );
                          
        xmd.omt(1).signal(i,2) =  xmd.omt(1).signal(i,2) + seed_noise(rand, noise);                             
    end

    % MJH processing 15/06/07: remove mean
    xmd.omt(1).signal(:,2) = xmd.omt(1).signal(:,2) - mean(xmd.omt(1).signal(:,2));

    disp(['Spectrum ======================================']);
    XMD_sample.omt = spec(xmd.omt, winl, norm);

    XMD.omt.F(:, kk) = XMD_sample.omt.F(:,1);
    XMD.omt.t(kk)    = tmax * (kk-1);
    XMD.omt.f        = XMD_sample.omt.f;
    XMD.omt.dt       = XMD_sample.omt.dt;
    XMD.omt.df       = XMD_sample.omt.df;
    XMD.omt.data     = XMD_sample.omt.data;
    XMD.omt.phi      = XMD_sample.omt.phi;

end % kk



it_min = min(find(XMD.omt(1).t>=tmin));
it_max = max(find(XMD.omt(1).t<=Nsamples * tmax));
Nf     = size(XMD.omt(1).f,2)
for i=1:Nf
    XMD.omt(1).Fmean(i) = mean(abs(XMD.omt(1).F(i,it_min:it_max)));
end;

figure
fs = 14;
lw = 1.5;
set(gca,'FontSize',fs, 'LineWidth',lw,'Box','on','TickLength',[0.02 0.02],'TickDir','in'); hold on;
fnorm = 1e+3
h1 = plot(XMD.omt(1).f/fnorm, XMD.omt(1).Fmean,'k', 'LineWidth',lw); hold on
set(gca,'YScale','log');
xlabel(['f [kHz]']);
axis([ 0 60 min(abs(XMD.omt(1).Fmean)) max(abs(XMD.omt(1).Fmean))]);


% replicate for negative frequency - taking complex conjugate
NXf       = length(XMD.omt.f);
NXt       = length(XMD.omt.t);

YMD.omt.f = -fliplr(XMD.omt.f);

YMD.omt.f(NXf+2: 2*NXf) = XMD.omt.f(2:NXf);
for i=1:NXt
    YMD.omt.F(1:NXf,i)       = flipud(XMD.omt.F(:,i))';
    YMD.omt.F(NXf+1:2*NXf,i) = XMD.omt.F(:,i);
end;    

YMD.omt.data = XMD.omt.data;
YMD.omt.phi  = XMD.omt.phi;
YMD.omt.t    = XMD.omt.t;
YMD.omt.dt   = XMD.omt.dt;
YMD.omt.df   = XMD.omt.df;

tnorm = 1e-3
fnorm = 1e+3
render_example(XMD.omt(1),tnorm, fnorm)

[bispec] = bispectral_bif(YMD)

if_min = min(find(YMD.omt.f >= 12e+3))
if_max = min(find(YMD.omt.f >= 18e+3))
if_pk  = min(find(YMD.omt.f >= 30e+3))
figure
time    = YMD.omt.t;
ph_diff = angle(YMD.omt.F(if_max,:)) + angle(YMD.omt.F(if_min,:)) - angle(YMD.omt.F(if_pk,:));
iph_neg = find(ph_diff <0);
ph_diff(iph_neg) = ph_diff(iph_neg) + 2*pi;

% plot(time, ph_diff);
plot(YMD.omt.f, angle(YMD.omt.F(:,1)));



f1  = w1/(2*pi);
f2  = w2/(2*pi);
f3  = w3/(2*pi);

ifk = min(find(bispec.fk(:,1)>=f1))
ifl = min(find(bispec.fl(1,:)>=f3))


figure
fs = 14;
lw = 1.5;
set(gca,'FontSize',fs, 'LineWidth',lw,'Box','on','TickLength',[0.02 0.02],'TickDir','in'); hold on;

subplot(2,1,1); hold on;
set(gca,'FontSize',fs, 'LineWidth',lw,'Box','on','TickLength',[0.02 0.02],'TickDir','in'); hold on;

plot(bispec.fl(1,:), abs(bispec.Bn(ifk,:)),'b')
plot(bispec.fk(:,1), abs(bispec.Bn(:,ifl)),'r')
ylabel(['|B|']);

subplot(2,1,2); hold on;
set(gca,'FontSize',fs, 'LineWidth',lw,'Box','on','TickLength',[0.02 0.02],'TickDir','in'); hold on;
phl = angle(bispec.Bn(ifk,:))/pi * 180;
phk = angle(bispec.Bn(:,ifl))/pi * 180;
Nph = length(phl);
ph_jump = 180;
for i=1:Nph
    while phl(i)<-ph_jump         phl(i) = phl(i) + 2*ph_jump;    end
    while phl(i)>+ph_jump         phl(i) = phl(i) - 2*ph_jump;    end    
    while phk(i)<-ph_jump         phk(i) = phk(i) + 2*ph_jump;    end
    while phk(i)>+ph_jump         phk(i) = phk(i) - 2*ph_jump;    end    
end
plot(bispec.fl(1,:), phl,'b')
plot(bispec.fk(:,1), phk,'r')
f1 = min(bispec.fl(1,:))
f2 = max(bispec.fl(1,:))

pred.ph1 = PHI10 + PHI20 - PHI30;
pred.ph2 = (-PHI10 + PHI30 - PHI20);
pred.ph3 = (-PHI20 + PHI30 - PHI10);

while pred.ph1<-pi     pred.ph1 = pred.ph1 + 2*pi; end
while pred.ph1>pi      pred.ph1 = pred.ph1 - 2*pi; end
while pred.ph2<-pi     pred.ph2 = pred.ph2 + 2*pi; end
while pred.ph2>pi      pred.ph2 = pred.ph2 - 2*pi; end
while pred.ph3<-pi     pred.ph3 = pred.ph3 + 2*pi; end
while pred.ph3>pi      pred.ph3 = pred.ph3 - 2*pi; end


plot([f1 f2],180/pi * [pred.ph1 pred.ph1],'b--');
plot([f1 f2],180/pi * [pred.ph2 pred.ph2],'r--');
plot([f1 f2],180/pi * [pred.ph3 pred.ph3],'r--');

xlabel(['f [Hz]']);
ylabel(['<B']);

return

