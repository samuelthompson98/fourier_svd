...
for i=1:Ncoils
  if x(i).data
    ...
    if isempty(varargin)
       [X(i).F, X(i).f, X(i).t, X(i).dt, X(i).df] = getspec(xin,1);
    elseif (size(varargin,2)==1)
       winl = varargin{1};    
       [X(i).F, X(i).f, X(i).t, X(i).dt, X(i).df] = getspec(xin,1, winl);
    ...
    elseif (size(varargin,2)==3)  
       ...
       norm = varargin{2};
       test = varargin{3};
       [X(i).F, X(i).f, X(i).t, X(i).dt, X(i).df] = getspec(xin,1, winl, norm, test);        
    elseif (size(varargin,2)==5)  
       ...
       flow = varargin{4};
       fhigh= varargin{5};
       [X(i).F, X(i).f, X(i).t, X(i).dt, X(i).df] = getspec(xin,1, winl, norm, test, flow, fhigh)       
   end;
