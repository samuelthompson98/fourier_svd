...
function [h]=render_nmode_M3a(X,tnorm, fnorm, n_alias, snr, fc)
...
[Z,hfig]  = pltn_M3data1(X, n_alias, hpos(1), 4, filt);

stop = 0;
while ~stop
    hpos      = wait(hlin)
    [Z, hfig] = pltn_M3data1(X, n_alias, hpos(1), 4, filt, hfig);
    if hpos(1,2) < 0
        stop = 1;
    end
