...
% deliberateyly introduce freqency blur - ie. ramp frequency over interval
ft = 99 + t/2;
Nt = length(t);
for i=1:Nt
    x(i) = a1 * cos(2*pi*ft(i)*t(i) + PHI1);
end;
...
ph_delta= delta_f * tmax/4 * 2 * pi;    % maximum average phase advance of f1 wrt f(imax) over window
