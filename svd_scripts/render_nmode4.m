...
function [h]=render_nmode4(X,tnorm, fnorm, n_alias, snr, fc)
...
% In practice, slice out from 1 to i_cut will never apply, as 
% low frequency components will always be larger.
...
% axes(hspec);
% axis([tnorm*min(t) tnorm*max(t+twin) fnorm*min(fl) fnorm*max(fl)]);
