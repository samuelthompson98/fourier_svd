...
beta(4) = 8 * NTM.A(4)*2*pi*NTM.f(4)* Ncoils * Area - 5 * beta(5)* lambda;
beta(3) = 4 * NTM.A(3)*2*pi*NTM.f(3)* Ncoils * Area - 5/4 * beta(5) - 4 * beta(4) * lambda - 10 * beta(5) * lambda^2;
beta(2) = 2 * NTM.A(2)*2*pi*NTM.f(2)* Ncoils * Area - 8/8 * beta(4) - ...
         (3* beta(3) + 5 * beta(5)) * lambda -  6* beta(4) * lambda^2 - 10  * beta(5) * lambda^3;
    
beta(1) = 1 * NTM.A(1)*2*pi*NTM.f(1)* Ncoils * Area - 12/16 * beta(3) - 10/16 * beta(5) - ...
          (2 * beta(2) + 3* beta(4)) * lambda - (3 * beta(3) + 15/2* beta(5)) * lambda^2 - ...
          4 * beta(4) * lambda^3 - 5* beta(5) * lambda^4;
      
f2 = CAE.A(1)*2*pi*CAE.f(1)* Ncoils * Area / ...
    (beta(1) + 3/2 * beta(3) + 15/8 * beta(5)  +  lambda * (2 * beta(2) + 6 * beta(4)) + ...
    lambda^2 * (3 * beta(3) + 15 * beta(5)) + lambda^3 * 4 * beta(4) + lambda^4 * 5 * beta(5));
...
for i=1:Nt
    ...
    xmd.omt(1).signal(i,2) =  lambda + f1 * cos( - n1* xmd.omt(1).phi  + w1*t(i) + PHI1) + ...
